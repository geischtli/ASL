...
for m = ms
    ...
    rho0 = calculate_rho0(rho, m);
    l = ((m.*rho).^m./(factorial(m)*(1-rho))).*rho0;
    Er = 1./(double_service_rate).*(1 + (l)./(m.*(1-rho)));
    plot(arrival_rate, Er, 'r',  'linewidth', 2, 'color', 'red', ...
        'linestyle', ':')
    err = norm(response_time - Er)
end
legend('Model for 1MW', 'Model for 2 MWs', 'Measurements 2 MWs', ...
    'Measurements 1 MW', 'location', 'northwest')
