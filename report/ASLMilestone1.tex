...
\end{lstlisting}
The generation of the actual requests is fully done in code. This is done, such that no file access is needed, which typically tends to be slow, the files are big and it's generally less flexible. The procedure is split into two parts:
\begin{enumerate}
...
\end{lstlisting}
This adds the array [\textit{HANDSHAKE}, \textit{CREATE\_QUEUE}] once to the \textit{requestList}, which is later accessed by the client.
	\item When running, the client accesses this list and dynamically creates a request object of the corresponding type, passing the currently available field values (stored in a session object each client manages, where earlier answers of queries are written into) as parameters to the constructor. In code this is as simple as:
\begin{lstlisting}[basicstyle=\small, language=Java]
Request req = RequestBuilder.getRequest(
    requestList.get(reqCounter, client_session));
\end{lstlisting}, where \textit{getRequest} is simply a switch-case about the inputted request type.
\end{enumerate}
...
The deployment of a client is done in only a few steps, since the whole source code and all needed files are stored in a git repository:
\begin{enumerate}
